  ...
  for (const mob of mobs.values()) {
    ...
    if (target && now < mob.chaseUntil) {
      ...
      if ((shape === 'spider' || shape === 'orumcek') && distance < 300 && distance > 70 && now >= (mob.nextAbilityAt || 0)) {
        ...
        mob.chaseUntil = now + MOB_CHASE_TIMEOUT;
        mob.state = 'attack';
        const dashDist = Math.min(220, distance - 40);
        mob.x += Math.cos(targetAngle) * dashDist;
        mob.y += Math.sin(targetAngle) * dashDist;
        const dashDmg = 45;
        ...
        mob.chaseUntil = now + MOB_CHASE_TIMEOUT;
        mob.state = 'attack';
        const chargeDist = Math.min(180, distance - 30);
        mob.x += Math.cos(targetAngle) * chargeDist;
        mob.y += Math.sin(targetAngle) * chargeDist;
        const trampleDmg = 80;
        ...
        mob.chaseUntil = now + MOB_CHASE_TIMEOUT;
        mob.state = 'attack';
        const pounceDist = Math.min(190, distance - 30);
        mob.x += Math.cos(targetAngle) * pounceDist;
        mob.y += Math.sin(targetAngle) * pounceDist;
        const pounceDmg = 60;
...
setInterval(() => {
  ...
  if (botList.size < targetBotCount) {
    createBot();
  } else if (botList.size > targetBotCount) {
    for (const [id, bot] of botList) {
      if (botList.size <= targetBotCount) break;
      let nearReal = false;
      for (const socket of io.sockets.sockets.values()) {
        const rp = players.get(socket.id);
        if (rp && Math.hypot(rp.x - bot.x, rp.y - bot.y) < 1600) {
          nearReal = true;
          break;
        }
      }
      if (!nearReal) removeBot(id);
    }
  }
  ...
  for (const bot of botList.values()) {
    ...
    // 0. Trap Check: If trapped by an enemy trap, completely freeze (cannot move, cannot attack)
    if (bot.trappedBy) {
      ...
      if (b && (b.hp ?? 100) > 0 && now < (bot.trappedUntil || 0)) {
        ...
        bot.trappedBy = null;
        bot.trappedUntil = 0;
        io.emit('players', { [bot.id]: { trappedBy: null, trappedX: null, trappedY: null } });
      }
    }

    // 1. Survival Check: Heal with apples if low on HP (human player parity)
    if (bot.hp < 210 && bot.apples > 0 && now - (bot.lastAppleAt || 0) > 340) {
      bot.apples--;
      bot.hp = Math.min(bot.maxHp, bot.hp + 32);
      bot.hpSeq = (bot.hpSeq || 0) + 1;
      bot.lastAppleAt = now;
      io.emit('players', { [bot.id]: { hp: bot.hp, hpSeq: bot.hpSeq } });

      // Defensive tactical spike drop when wounded in close combat
      if (bot.target && bot.target.type === 'player' && bot.wood >= 20 && now - (bot.lastBuildAt || 0) > 2400) {
        bot.lastBuildAt = now;
        bot.wood -= 20;
        const bX = Math.round(bot.x + Math.cos(bot.angle) * 65);
        const bY = Math.round(bot.y + Math.sin(bot.angle) * 65);
        ...
        const cellKey = `${Math.floor(bX / BUILDING_CELL_SIZE)},${Math.floor(bY / BUILDING_CELL_SIZE)}`;
        const bucket = buildingGrid.get(cellKey);
        if (bucket) bucket.push(defSpike);
        else buildingGrid.set(cellKey, [defSpike]);
        io.emit('build', { id: bId, building: { ...defSpike } });
      }
    }

    // 2. Target Selection / Aggro update: Realistic aggro range (no chasing across map)
    if (bot.target && bot.target.type === 'player') {
      const curDist = Math.hypot(bot.target.x - bot.x, bot.target.y - bot.y);
      if (curDist > 450 || !bot.target.entity || (bot.target.entity.hp ?? 0) <= 0 || bot.target.entity._dead) {
        // Player ran away or died - drop aggro immediately! No endless chasing!
        bot.target = null;
        bot.targetChangeAt = now + 1500;
      }
    }

    if (!bot.target || now > (bot.targetChangeAt || 0)) {
      let bestTarget = null;
      let bestDist = 400; // Realistic engagement radius

      // 2a. Squad Defense: Protect nearby clanmate being attacked
      if (bot.clanId) {
        for (const [otherId, ally] of players) {
          if (otherId === bot.id || !ally || (ally.hp ?? 0) <= 0) continue;
          if (ally.clanId === bot.clanId || ally.team === bot.team) {
            const dAlly = Math.hypot(ally.x - bot.x, ally.y - bot.y);
            if (dAlly < 450 && ally._lastAttackedBy) {
              const attacker = players.get(ally._lastAttackedBy);
              if (attacker && (attacker.hp ?? 0) > 0 && attacker.clanId !== bot.clanId) {
      ...
      // 2b. Nearest Hostile Player within local vision range
      if (!bestTarget) {
      ...
      // 2c. Resource gathering when low on resources or peaceful
      if (!bestTarget && (bot.wood < 80 || bot.stone < 40)) {
        for (let i = 0; i < Math.min(serverResources.length, 60); i++) {
      ...
      // 2d. Peaceful Roam
      if (!bestTarget) {
        const roamDist = 200 + Math.random() * 260;
      ...
      bot.targetChangeAt = now + 2400 + Math.random() * 1600;
    }
    ...
    if (bot.wood >= 40 && bot.stone >= 20 && (!bot.target || bot.target.type === 'roam') && now - (bot.lastBaseActionAt || 0) > 3000) {
      if (!bot.baseX) {
      ...
      if (bot.baseStep === 0 && bot.wood >= 40 && bot.stone >= 20) {
        ...
        pX = Math.round(bot.baseX + 42);
        pY = Math.round(bot.baseY + 42);
      }
      ...
      if (placeType !== null) {
        ...
        if (bot.wood >= wCost && bot.stone >= sCost) {
          ...
          const cellKey = `${Math.floor(pX / BUILDING_CELL_SIZE)},${Math.floor(pY / BUILDING_CELL_SIZE)}`;
          const bucket = buildingGrid.get(cellKey);
          if (bucket) bucket.push(newBld);
          else buildingGrid.set(cellKey, [newBld]);
    ...
    if (bot.target) {
      ...
      while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
      bot.angle += angleDiff * 0.35;

      // Smart Weapon Selection: Axe (1) for resources, Sword (2) for PvP
      ...
      if (bot.target.type === 'player') {
        if (dist > 450) {
          // Beyond aggro range: stop chasing immediately
          bot.target = null;
          bot.vx = 0;
          bot.vy = 0;
        } else if (dist > 95) {
          // Approach target at normal speed
          moveSpeed = baseSpeed;
          moveAngle = bot.angle;
          bot.vx = Math.cos(moveAngle) * moveSpeed;
          bot.vy = Math.sin(moveAngle) * moveSpeed;
        } else if (dist <= 95 && dist >= 55) {
          // In combat: Tactical Circle-Strafing around opponent (human player behavior)
        ...
        for (const b of nearbyBuildings(bot.x, bot.y, botRad + 120)) {
          ...
          if (b.type === 6 && b.ownerId !== bot.id && (!bot.clanId || b.ownerClanId !== bot.clanId)) {
            ...
            if (tdx * tdx + tdy * tdy <= trapTriggerR * trapTriggerR) {
              ...
              bot.x = b.x; bot.y = b.y;
              bot.vx = 0; bot.vy = 0;
              io.emit('trap_triggered', { buildingId: b.id, victimId: bot.id, bId: b.id });
              io.emit('players', { [bot.id]: { trappedBy: b.id, trappedX: b.x, trappedY: b.y, x: b.x, y: b.y } });
              break;
            }
          }
          ...
          if (col && col.collided && pass === 0) {
            ...
            // Enemy Spike contact damage
            if (b.type === 3 && b.ownerId !== bot.id && (!bot.clanId || b.ownerClanId !== bot.clanId) && now - (bot.lastSpikeHit || 0) > 650) {
              bot.lastSpikeHit = now;
              const spikeDmg = [35, 55, 80, 120][b.tier || 0] || 35;
              applyPlayerDamage(bot, spikeDmg);
              io.emit('players', { [bot.id]: compactState(bot) });
              if (bot.hp <= 0) {
                onPlayerDeath(bot.id);
                const owner = players.get(b.ownerId);
                if (owner) {
                  owner.kills = (owner.kills || 0) + 1;
                  owner.score = (owner.score || 0) + 200;
                }
                io.emit('player_dead', { id: bot.id });
                break;
              }
            }
          }
      ...
      if (dist <= 125 && !bot.trappedBy) {
        ...
        if (now - (bot.lastSwingAt || 0) >= swingCooldown) {
          ...
          // Broadcast attack event - triggers identical weapon & body swing arc on client!
          ...
          io.emit('playerAttack', bot.id);

          if (bot.target.type === 'player' && bot.target.entity) {
            ...
            if (facingDiff < Math.PI * 0.42 && (!targetPlayer.clanId || targetPlayer.clanId !== bot.clanId)) {
              ...
              io.emit('players', { [targetPlayer.id]: compactState(targetPlayer) });
          ...
          } else if (bot.target.type === 'resource') {
